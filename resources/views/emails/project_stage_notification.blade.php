...
<table align="center" cellpadding="0" cellspacing="0" width="600"
    ...
    <tr>
        <td style="padding:30px;">
            ...
            <p style="margin:0 0 10px;"><strong>Клієнт:</strong> {{$project->client?->name ?? '—'}}</p>
            <p style="margin:0 0 10px;"><strong>Назва проєкту:</strong> {{$project->name}}</p>

            <p style="margin:0 0 10px;"><strong>Поточна стадія:</strong>
                <span style="background:#007bff; color:#fff; padding:3px 8px; border-radius:4px;">
                    {{$project->status?->name ?? '—'}}
                </span>
            </p>

            <p style="margin:0 0 20px;"><strong>Дата нагадування:</strong> {{ now()->format('d.m.Y H:i') }}</p>
